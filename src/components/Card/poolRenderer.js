...
const PoolRenderer = (props) => {
  ...
  const {
    pool: idoInfo,
    pool: { start, end, metadata, idoAddress, tokenName, tokenSymbol, softCap, hardCap, progress },
  } = props;
      ...
      >
        <s.UpperCard fd="row" jc="space-between" ai="center">
          ...
          <s.Container flex={3} ai="center" style={{ paddingLeft: 5 }}>
            <s.TextDescriptionEllipsis style={{ textAlign: 'center' }} fs={'26px'}>
              {tokenName}
        ...
        <s.Container fd="row">
          <s.Container ai="center" flex={1}>
            ...
            {BigNumber(contract.web3.utils.fromWei(softCap)).toFormat(2) + ' ' + baseCurrencySymbol}
          </s.Container>
          <s.Container ai="center" flex={1}>
            <s.TextID fullWidth>Hard cap</s.TextID>
            {BigNumber(contract.web3.utils.fromWei(hardCap)).toFormat(2) + ' ' + baseCurrencySymbol}
          </s.Container>
        </s.Container>
        <s.SpacerSmall />
        {!hasEnded && (
          <>
            <s.TextID>{isStarted ? 'End in' : 'Start in'}</s.TextID>
            <Countdown date={isStarted ? parseInt(end) * 1000 : parseInt(start) * 1000} />
          </>
        )}
